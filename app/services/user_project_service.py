...
def create_project(db: Session, project: schemas.ProjectCreate, user_id: int):
    """Create a new project and assign admin permission to the creating user."""
    db_project = models.Project(name=project.name, description=project.description, image_path=project.image_path, is_public=project.is_public)
    db.add(db_project)
...
def delete_project(db: Session, project_id: int):
    """Delete a project and its associated permissions."""
    db_project = db.query(models.Project).filter(models.Project.id == project_id).first()
    if db_project:
...
def get_user_projects(db: Session, user_id: int):
    """Get projects associated with a user (explicit permission or public)."""
    ...
    user_permissions = db.query(models.UserProjectPermission).filter(models.UserProjectPermission.user_id == user_id).all()
...
def get_user_project_permission(db: Session, user_id: int, project_id: int):
    """Get a specific user's permission for a project."""
...
def create_user_project_permission(db: Session, permission: schemas.UserProjectPermissionCreate):
    """Create a new user project permission."""
...
def update_user_project_permission(db: Session, user_id: int, project_id: int, permission_level: str):
    """Update a user's permission level for a project."""
...
def delete_user_project_permission(db: Session, user_id: int, project_id: int):
    """Delete a user's permission for a project."""
    db_permission = db.query(models.UserProjectPermission).filter(
        models.UserProjectPermission.user_id == user_id,
        models.UserProjectPermission.project_id == project_id
    ).first()
